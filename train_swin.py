...
def train(student, train_loader, test_loader, unlabelled_train_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        # if epoch > 100:
        #     torch.save(save_dict, args.model_path + f"_epoch_{epoch}")
        #     args.logger.info("model saved to {}.".format(args.model_path + f"_epoch_{epoch}"))
        # if epoch > 100:
        torch.save(save_dict, args.model_path)
        args.logger.info("model saved to {}.".format(args.model_path))
        with open(os.path.dirname(args.model_path) + "/stat_{}.txt".format(args.dataset_name), 'w') as f: 
            f.write('Train Accuracies: All {:.4f} | Old {:.4f} | New {:.4f}'.format(all_acc, old_acc, new_acc))
        ...
        if epoch == 199:
            torch.save(save_dict, args.model_path + "_200")
            args.logger.info("model saved to {}.".format(args.model_path))
